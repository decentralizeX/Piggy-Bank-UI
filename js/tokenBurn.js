...
//farm contract
		  ...
		  /*
		   fetch(apiURL+'api/governorBalance.txt', {cache: "no-cache"}).then(function (response) {
			...
			document.getElementById("gov-balance").innerHTML = Number(html)
		})
		*/
			...
			document.getElementById('gov-balance').innerHTML = `						
			<select class="form-control" id="burnFrom">
						<option value="0">Governor Wallet ( Available:  `+Number(html).toLocaleString()+` `+tokenTicker+` )</option></select></div></select>
                    `
		})
...
fetch(apiURL+'proposals/tokenBurn.html', {cache: "no-cache"}).then(function (response) {
			return response.text();
		}).then(function (html) {
			document.getElementById("tokenBurnTable").innerHTML = html
		})
		...
		async function requestTreasuryWithdraw() {
			...
			let pricer = 0; 
				...
				pricer = BigInt(entry) * BigInt(10**18);
			...
			if(entry === "" || commitValue === "") {
						...
						Swal.fire('Error!',
						  'A minimum commitment of ' + minCostToVote.toLocaleString() + ''+tokenTicker+' is required to request a token burn!',
						  'warning'
						)
			} else {
			  await auth();
			...
			const displayText = "You will sacrifice <strong>" + commitValue.toLocaleString() + " "+tokenTicker+"</strong> tokens to initiate the proposal";
			  Swal.fire({
				...
				const doIt = await transaction.proposeGovernorTransfer(commit,pricer,true,0,delayBEval);
...
async function voteGovernorTransfer(propID, forOrAgainst, isRejectPossible, requiredToReject) {
	   ...
	   let displayText = '<img src="favicon/favicon-32x32.png" width="28px" height="28px"> '+tokenTicker+' to Sacrifice:</br><input id="sacAmount" autocapitalize="off" class="swal2-input" placeholder="" type="text" style="display: flex;"></br><small>' + decisionText + '</small> ';
