...
if(localStorage.getItem('claimedFaucet') !== null) {
	localStorage.removeItem('claimedFaucet')
	Swal.fire({
                    title: '<strong>Welcome to the Staking Dashboard!<strong>',
                    html: 'You have succesfully claimed the '+tokenTicker+' tokens from the faucet!</br></br> The contract has sent you:</br> <li>'+tokenTicker+' tokens to wallet</li><li>A Mature 1-month miner(ready for roll-over)</li><li>2 Stakes in Each Miner Option</li> </br></br> Now you can test all of the features</br> <li>roll over/extend mature miner - protocol pays you a rollover bonus</li><li> Extend(change) The mining duration of an active miner to earn higher rewards</li><li>transfer miner to another wallet</li><li> Check your governing power(share in the network)</li></br> ... and much more. Enjoy!',
                    icon: 'info',
                    focusConfirm: false,
...
var savedContent = "";
var header = '<table id="latestActions" class="table table-striped"><thead><tr><th>Miner(CD) Duration</th><th>Tokens Staked</th><th>Earned <small>(since last action)</small></th><th>Time Served </th><th>Penalty</th><th>Actions</th></tr></thead><tbody>'
...
if(isTestnet) {
							let actionToLog = 'viewStakesCards'
...
async function displayStakes(nrOf, userAddress, contractAddress, duration, serveTime, isProxy) {
    ...
    for (let i = 0; i < nrOf; i++) {
        ...
        lastMiner = Number(BigInt(stakeInfo[1]));
        mandatoryServe = Number(BigInt(stakeInfo[4]));

        tokensEarned = tokensStaked - Number(BigInt(stakeInfo[2]) / BigInt(10 ** 18));
		if(duration=="1 Month Miner") {
		console.log(" TOKENS STAKED: " + tokensStaked + " ...TOKENS EARNED: " + tokensEarned) }
        if (tokensEarned < 0) {
            tokensEarned = 0
        }

        progressAmount = timestamp - (lastMiner + serveTime);
        if (progressAmount < 0) {
            progressAmount = timestamp - lastMiner //seconds served
            progressAmount = Math.round(progressAmount / serveTime * 100)
        } else {
            progressAmount = 100
        }
        if (mandatoryServe == 0) {
            progress = '<div class="progress" style="background-color: #26262a;" onclick="stakeExpiry('+(Number(lastMiner*1000)+Number(serveTime*1000))+')" style="cursor: pointer;"><div class="progress-bar bg-success" role="progressbar" style="width: ' + progressAmount + '%;" aria-valuenow="' + progressAmount + '" aria-valuemin="0" aria-valuemax="100"></div></div>'
        } else {
            progressAmount2 = timestamp - (lastMiner + mandatoryServe)
            if (progressAmount2 < 0) {
                progressAmount2 = Math.round((timestamp - lastMiner) / mandatoryServe * 100)
            } else {
                progressAmount2 = 100
            }
            progress = '<div class="progress" style="background-color: #26262a;" onclick="stakeExpiry('+(lastMiner+serveTime)+')" style="cursor: pointer;"><div class="progress-bar bg-success" role="progressbar" style="width: ' + progressAmount + '%;" aria-valuenow="' + progressAmount + '" aria-valuemin="0" aria-valuemax="100"></div></div>'
            progress += '<div style="display:inline;cursor: pointer;" onclick="mandatoryServe(' + mandatoryServe + ', ' + (timestamp - lastMiner) + ')"><small>Mandatory Serve <i id="delayInfo" class="mdi mdi-information-outline"></i></div></small></br><div class="progress" style="background-color: #26262a;"><div class="progress-bar bg-info" role="progressbar" style="width: ' + progressAmount2 + '%" aria-valuenow="' + progressAmount2 + '" aria-valuemin="0" aria-valuemax="100"></div></div>'
        }
        penalty = calculatePenalty(contractAddress, (timestamp - lastMiner), serveTime);
		let additionalContent = '';
		let rollOverContent = '';
		let transferStakeContent = '';
		let withinGrace = false;
        if (!isProxy) {
            let stakeShares = BigInt(stakeInfo[0]);
             //don't display additional content on 5yr pool(ability to transfer to another pool)
            if (contractAddress != pool5Year) {
                additionalContent = '<a class="dropdown-item" href="#" onclick="hopPools(' + tokensStaked + ', \'' + contractAddress + '\', \'' + stakeShares.toString() + '\', ' + i + ');">Transfer Miner To Another Pool</a><div class="dropdown-divider"></div>'
            } else {
                additionalContent = ''
            }
            if (progressAmount < 100) { //can't extend
				transferStakeContent = '<a class="dropdown-item" href="#" onclick="transferToAnotherWallet(\'' + contractAddress + '\', ' + i + ', ' + tokensStaked + ', \'' + (BigInt(stakeInfo[0])) + '\', \'' + sharePrice + '\');">Transfer Miner To Another Wallet</a><div class="dropdown-divider"></div>'
                savedContent = savedContent + ' <tr><td>' + duration + '</td> <td>' + Math.round(tokensStaked).toLocaleString() + ' '+tokenTicker+' <small>(<font color="green">$</font>' + Math.round(tokensStaked * price).toLocaleString() + ')</small></td> <td id="earnedCell'+consecutiveStake+'"><span id="earnedSpan'+consecutiveStake+'">' +Math.round(Number(tokensEarned)).toLocaleString() + '</span> '+tokenTicker+' <small>(<font color="green">$</font>' + ((Number(tokensEarned) * price * 100) / 100).toLocaleString() + ')</small></td><td>' + progress + '</td> <td>' + (Math.round(penalty * 100) / 100) + ' %</td><td><div class="dropdown"><button type="button" class="btn btn-sm btn-outline-secondary dropdown-toggle" id="dropdownMenuIconButton3" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"></button><div class="dropdown-menu" aria-labelledby="dropdownMenuIconButton3">' + additionalContent + '<a class="dropdown-item" href="#" onclick="transferToAnotherWallet(\'' + contractAddress + '\', ' + i + ', ' + tokensStaked + ', \'' + (BigInt(stakeInfo[0])) + '\', \'' + sharePrice + '\');">Transfer Miner To Another Wallet</a><div class="dropdown-divider"></div><a class="dropdown-item" href="#" onclick="withdraw(\'' + contractAddress + '\', false,  ' + i + ', ' + tokensStaked + ', \'' + (BigInt(stakeInfo[0])) + '\', \'' + sharePrice + '\');">Withdraw</a></div></div></td>'
				...
				transferStakeContent = '<a class="dropdown-item" href="#" onclick="transferToAnotherWallet(\'' + contractAddress + '\', ' + i + ', ' + tokensStaked + ', \'' + (BigInt(stakeInfo[0])) + '\', \'' + sharePrice + '\');">Transfer Miner To Another Wallet</a><div class="dropdown-divider"></div>'
				rollOverContent+= '<div class="badge badge-outline-success" onclick="rollOver(' + tokensStaked + ', \'' + contractAddress + '\', ' + i + ', ' + withinGrace + ');" style="cursor: pointer;">Roll-Over</div></br></br>'
				additionalContent=  '<a class="dropdown-item"><center><div class="badge badge-outline-success" onclick="rollOver(' + tokensStaked + ', \'' + contractAddress + '\', ' + i + ', ' + withinGrace + ');" style="cursor: pointer;">Roll-Over</div></center><div class="dropdown-divider"></div></a>' + additionalContent
                savedContent = savedContent + ' <tr><td>' + duration + '</td> <td>' + Math.round(tokensStaked).toLocaleString() + ' '+tokenTicker+' <small>(<font color="green">$</font>' + Math.round(tokensStaked * price).toLocaleString() + ')</small></td> <td id="earnedCell'+consecutiveStake+'"><span id="earnedSpan'+consecutiveStake+'">' +Math.round(Number(tokensEarned)).toLocaleString() + '</span> '+tokenTicker+' <small>(<font color="green">$</font>' + ((Number(tokensEarned) * price * 100) / 100).toLocaleString() + ')</small></td><td>' + progress + '</td> <td><div class="badge badge-outline-success" onclick="rollOver(' + tokensStaked + ', \'' + contractAddress + '\', ' + i + ', ' + withinGrace + ');" style="cursor: pointer;">Roll-Over</div></td><td><div class="dropdown"><button type="button" class="btn btn-sm btn-outline-secondary dropdown-toggle" id="dropdownMenuIconButton3" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"></button><div class="dropdown-menu" aria-labelledby="dropdownMenuIconButton3"><a class="dropdown-item" href="#" onclick="rollOver(' + tokensStaked + ', \'' + contractAddress + '\', ' + i + ', ' + withinGrace + ');">Rollover (Extend) Stake</a><div class="dropdown-divider"></div>' + additionalContent + '<a class="dropdown-item" href="#" onclick="transferToAnotherWallet(\'' + contractAddress + '\', ' + i + ', ' + tokensStaked + ', \'' + (BigInt(stakeInfo[0])) + '\', \'' + sharePrice + '\');">Transfer Miner To Another Wallet</a><div class="dropdown-divider"></div><a class="dropdown-item" href="#" onclick="withdraw(\'' + contractAddress + '\', ' + withinGrace + ',  ' + i + ', ' + tokensStaked + ', \'' + (BigInt(stakeInfo[0])) + '\', \'' + sharePrice + '\');">Withdraw</a></div></div></td>'
            }
        } else {
            savedContent = savedContent + ' <tr><td>' + duration + '</td> <td>' + Math.round(tokensStaked).toLocaleString() + ' '+tokenTicker+' <small>(<font color="green">$</font>' + Math.round(tokensStaked * price).toLocaleString() + ')</small></td> <td id="earnedCell'+consecutiveStake+'"><span id="earnedSpan'+consecutiveStake+'">' +Math.round(Number(tokensEarned)).toLocaleString() + '</span> '+tokenTicker+' <small>(<font color="green">$</font>' + ((Number(tokensEarned) * price * 100) / 100).toLocaleString() + ')</small></td><td>' + progress + '</td> <td>' + (Math.round(penalty * 100) / 100) + ' %</td>'
        }
		 boxes+= `
	 			     <div class="col-md-6 col-xl-4 grid-margin stretch-card pricing-card">
                        <div class="card border-primary border pricing-card-body block-containing-element" style="padding: 0px;"><div class="block-holder"><div class="block">
                          <div class="text-center pricing-card-head">
                            ...
                            <img src="img/pulse-dao.png" style="max-width: 175px">
                          </div>
              </br></br>
			
			  
			 <h3 class="font-weight-normal mb-4">` + Math.round(tokensStaked).toLocaleString() + ` (`+tokenTicker+`)</h3>
			 
			    <h7 class="font-weight-normal mb-4">Progress <small>(`+duration+`)</small>: `+rollOverContent+`
			  `+progress+`
			  </br><h7 class="font-weight-normal mb-4">Rewards Earned</br>
			 ` + `<span id="earnedCell`+consecutiveStake+`"><span id="earnedSpan`+consecutiveStake+`"> `+Math.round(Number(tokensEarned)).toLocaleString() + `</span> `+tokenTicker+` <small>(<font color="green">$</font>` + ((Number(tokensEarned) * price * 100) / 100).toLocaleString() + `)</small></span>` + ` </h7></br>
			 <!--<h9 class="font-weight-normal mb-4">Estimated Daily Interest:</br>
			 ` + tokensStaked.toLocaleString() + ` (`+tokenTicker+`)</h9></br>--></br>
						 ...
						 <div class="dropdown">
                            ...
                            <div class="dropdown-menu" x-placement="bottom-start" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(0px, 42px, 0px);">
                              `+additionalContent+`
							 `+transferStakeContent+`
                              <a class="dropdown-item" href="#" onclick="withdraw('` + contractAddress + `', ` + withinGrace + `,  ` + i + `, ` + tokensStaked + `, '` + (BigInt(stakeInfo[0])) + `', '` + sharePrice + `');"><center>End Miner (Withdraw)</center></a>
...
function mandatoryServe(timeToServe, timeServed) {
    ...
    Swal.fire({ // + (Math.round((timeToServe - timeServed) / 86400)) + ' days
        ...
        html: 'This miner has a mandatory time lock of <strong>' + (Math.round(timeToServe / 86400)) + ' days</strong>.</br> Miner has served <strong>' + (Math.round(timeServed / 86400)) + ' days</strong> so far. </br></br><strong> ' + percentComplete + '%</strong> complete <strong>(' + timeLeft + '</strong> left).</br></br><small></small>Normal miners can be withdrawn at any time. If withdrawn prematurely, penalties will apply. "Mandatory serve" is an additional option to irreversibly lock the miner. The miner can not be withdrawn until mandatory time has been served.',
        icon: 'info',
        focusConfirm: false,
        confirmButtonText: '<i class="fa fa-thumbs-up"></i> Close'
    })
...
async function extendStake(poolContract, poolInto, stakeID, bonusPaid, stringPool) {
    Swal.fire({
        ...
        html: 'You are rolling-over(extending) your miner into a <strong>' + stringPool + '</strong> Miner option. </br></br>You will receive a bonus of &asymp;<strong>' + bonusPaid + ' '+tokenTicker+'</strong> tokens</string> added to your miner.<div class="pixel-loader"></div>',
        focusConfirm: false,
        confirmButtonText: '<i class="fa fa-thumbs-up"></i> Close'
    })
    try {
        ...
        sleep(2100).then(() => {
            Swal.fire({
                ...
                html: '<h2 class="swal2-title" id="swal2-title" style="display: block;"><strong>Good Job.</strong></h2></br></br>Your miner has been successfully rolled-over and bonus of &asymp;<strong>' + bonusPaid + ' '+tokenTicker+'</strong> tokens have been added to the stake!</br></br> <small>You can close this window.</small>',
                icon: 'success',
                focusConfirm: false,
                confirmButtonText: '<i class="fa fa-thumbs-up"></i> Close'
            })
...
async function hopPools(tokens, fromAddress, stakeShares, stakeID) {
    let htmlToDisplay = 'You can always transfer your miner into a pool with longer-lockup period and get rewarded with higher APYs and more voting power.</br> Your time served, as well as principal and earned interest will be transferred into another time deposit option.</br>';
    ...
    try {
        ...
        sleep(2100).then(() => {
            Swal.fire({
                ...
                html: '<h2 class="swal2-title" id="swal2-title" style="display: block;"><strong>Good Job.</strong></h2></br></br>Your miner has been successfully transferred into new time deposit option!</br></br> <small>You can close this window.</small>',
                icon: 'success',
                focusConfirm: false,
                confirmButtonText: '<i class="fa fa-thumbs-up"></i> Close'
            })
	...
	if (localStorage.getItem('allStakes') !== null) {
		...
		for(let i=0; i < ourDataz.length; i++) {
			...
			for(let j=0; j < ourDataz[i].length; j++) {
				...
				//document.getElementById('earnedCell'+String(count)).innerHTML = Math.round(Number(tokensEarned)).toLocaleString() + ' OINK <small>(<font color="green">$</font>' + (Math.round(Number(tokensEarned) * price * 100) / 100).toLocaleString() + ')</small>'
...
function countUp(count, element, endNr, elementId) {
	...
	if(count < endNr) {
		...
		document.getElementById('earnedCell'+elementId).innerHTML = '<span id="earnedSpan'+elementId+'"</span>' +Math.round(endNr).toLocaleString() + '</span> '+tokenTicker+' <small>(<font color="green">$</font>' + ((Number(endNr) * Number(document.getElementById("DTX-price").innerHTML) * 100) / 100).toLocaleString() + ')</small>'
	}
}
