...
range.addEventListener('change', (e) => {
  ...
  client.textContent = clientValue / 10;
});
...
function showActive() {
	...
	c.style.display="block";
		   ...
		   fetch(apiURL+'api/governorTax.txt', {cache: "no-cache"}).then(function (response) {
			return response.text();
		}).then(function (html) {
			document.getElementById("governor-tax").innerHTML = Number(html)/100
		})
		
	fetch(apiURL+'proposals/governorTax.html', {cache: "no-cache"}).then(function (response) {
			return response.text();
		}).then(function (html) {
			document.getElementById("govTaxTable").innerHTML = html
		})
	
		async function requestGovernorTax() {
			const entry = parseInt(parseFloat(document.getElementById("client").innerText) * 100);
			const pricer = BigInt(entry);
			...
			if(entry === "" || commitValue === "") {
						...
						Swal.fire('Error!',
						  'A minimum commitment of ' + minCostToVote.toLocaleString() + ''+tokenTicker+' is required to request a treasury withdrawal!',
						  'warning'
						)
			} else {
			  await auth();
			...
			const displayText = "You will sacrifice <strong>" + commitValue.toLocaleString() + " "+tokenTicker+"</strong> tokens to initiate the proposal";
			  Swal.fire({
				...
				const doIt = await transaction.proposeGovTax(commit,pricer,delayBEval);
		...
		async function enforce(proposalNumber) {
			  await auth();
					...
					let msgSuccess= 'Proposal can be enforced. No '+tokenTicker+' required. </br></br><div class="pixel-loader"></div></br><small>Freedom comes with a price.</small> ';
						...
						try {
				const transaction = new ethers.Contract(farmsContract, ABI, provider.getSigner());
				const doIt = await transaction.executeGovTax(proposalNumber);
			  ...
			  await auth();
						...
						try {
				const transaction = new ethers.Contract(farmsContract, ABI, provider.getSigner());
				const doIt = await transaction.vetoGovTax(proposalNumber);
...
async function voteGovernorTax(propID, forOrAgainst, isRejectPossible, requiredToReject) {
...
Swal.fire({
	 ...
	 let sacrificeAmount = parseInt(document.getElementById('sacAmount').value);
				...
				var doIt = await transaction.voteGovTaxY(propID,(BigInt(sacrificeAmount) * BigInt(10**18)));
				...
				var transaction = new ethers.Contract(farmsContract, ABI, provider.getSigner());
				var doIt = await transaction.voteGovTaxN(propID,(BigInt(sacrificeAmount) * BigInt(10**18)),withUpdate);
							}
   ...
   function refreshGovTax() {
		...
		document.getElementById("btn-treasuryWithdraw").onclick = requestGovernorTax;
